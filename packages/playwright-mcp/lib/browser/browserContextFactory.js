...
class SharedContextFactory {
  ...
  async createContext(clientInfo, abortSignal, options) {
    if (!this._contextPromise) {
      ...
      // Use own abort signal — per-session aborts must NOT tear down shared infrastructure
      this._contextPromise = this._baseFactory.createContext(clientInfo, this._abortController.signal, options);
      // Invalidate cache when browser disconnects (extension WS death, CDP drop, etc.)
      // so the next tool call triggers a fresh context + relay + extension reconnection.
      this._contextPromise.then(({ browserContext }) => {
        const invalidate = () => { this._contextPromise = void 0; };
        browserContext.on('close', invalidate);
        const browser = browserContext.browser();
        if (browser) browser.on('disconnected', invalidate);
      }).catch(() => { this._contextPromise = void 0; });
    }
  ...
  async _dispose() {
    this._abortController.abort("SharedContextFactory disposed");
    this._baseFactory.stop?.();
  ...
  async listChromeTabs() {
    if (typeof this._baseFactory.listChromeTabs !== 'function')
      throw new Error("listChromeTabs is only available in extension mode");
    return this._baseFactory.listChromeTabs();
  }
  async switchChromeTab(strategy, params) {
    if (typeof this._baseFactory.switchChromeTab !== 'function')
      throw new Error("switchChromeTab is only available in extension mode");
    return this._baseFactory.switchChromeTab(strategy, params);
  }
}
