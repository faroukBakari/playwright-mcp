...
var import_util = require("../compat");
...
function configFromCLIOptions(cliOptions) {
  ...
  const config = {
    ...
    snapshot: cliOptions.snapshotMode || cliOptions.snapshotCompression ? {
      mode: cliOptions.snapshotMode,
      compression: cliOptions.snapshotCompression
    } : void 0,
...
function configFromEnv() {
  ...
  if (process.env.PLAYWRIGHT_MCP_SNAPSHOT_COMPRESSION)
    options.snapshotCompression = enumParser("--snapshot-compression", ["none", "algorithmic", "full"], process.env.PLAYWRIGHT_MCP_SNAPSHOT_COMPRESSION);
