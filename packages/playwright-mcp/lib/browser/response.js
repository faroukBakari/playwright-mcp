...
class Response {
  constructor(context, toolName, toolArgs, relativeTo, rawArgs) {
    this._results = [];
    this._errors = [];
    this._code = [];
  ...
  setIncludeSnapshot() {
    if (this._rawArgs?.includeSnapshot === false)
      return;
      return;
      return;
  ...
  async _build() {
    ...
    if (tabSnapshot && this._includeSnapshot !== "none") {
      ...
      const compressionMode = this._context.config.snapshot?.compression;
      if (compressionMode && compressionMode !== "none" && snapshot) {
        const { compressSnapshot } = require("./snapshotCompressor");
