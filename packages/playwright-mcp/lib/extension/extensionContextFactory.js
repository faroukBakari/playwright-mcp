...
const RELAY_PORT = import_cdpRelay.RELAY_PORT;
class ExtensionContextFactory {
  constructor(browserChannel, userDataDir, executablePath, forceNewTab) {
    ...
    // Eager relay creation — factory-scoped, survives context invalidation
    this._relayPromise = this._createRelay();
  }
  ...
  async _createRelay() {
    ...
    await (0, import_utils.startHttpServer)(httpServer, { port: RELAY_PORT });
    ...
    debugLogger(`CDP relay server started on port ${RELAY_PORT}, extension endpoint: ${cdpRelayServer.extensionEndpoint()}.`);
    return cdpRelayServer;
  }
  async _startRelay() {
    return this._relayPromise;
  }
  ...
  async switchChromeTab(strategy, params) {
    const relay = await this._startRelay();
    return relay.selectAndAttach(strategy, params);
  }
  stop() {
    if (this._relayPromise) {
      this._relayPromise.then(relay => relay.stop()).catch(() => {});
